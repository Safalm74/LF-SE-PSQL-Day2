...
--authors
create table
	if not exists bookStore.Authors (
...
--Publishers 
create table
	if not exists bookStore.Publishers (
...
--Customers 
create table
	if not exists bookStore.Customers (
...
--Orders 
create table
	if not exists bookStore.Orders (
...
--book authors
create table
	if not exists bookStore.Book_Authors (
		book_id int,
		author_id int,
...
--order items
create table
	if not exists bookStore.Order_Items (
		book_id int,
		order_id int,
		foreign key (book_id) references bookStore.Book (book_id),
...
--Authors
insert into
	bookstore.authors (author_name, birth_date, nationality)
values
...
--Publishers
insert into
	bookstore.Publishers (publisher_name, country)
values
	('A', 'Nepal'),
	('B', 'India'),
	('C', 'Japan');
...
--Customers
insert into
	bookstore.Customers (customer_name, email, address)
values
...
--book
insert into
	bookstore.book (title, genre, publisher_id, publication_year)
values
...
--book authors
insert into
	bookstore.book_authors (book_id, author_id)
values
	(1, 3),
	(2, 2),
	(3, 1),
...
--orders
insert into
	bookstore.orders (order_date, customer_id, total_amount)
values
	('2022-03-03', 1, 1000),
	('2022-03-03', 1, 2000),
	('2022-03-03', 1, 3000),
...
--order items
insert into
	bookstore.order_items (book_id, order_id)
values
	(1, 1),
	(1, 2),
	(2, 3),
